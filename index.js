...
const { ServerEventListener } = require('./client-to-server-events');
...
if (require.main === module) {
  browserRunner('https://code.visualstudio.com')
    .then((result) => {
      console.log('\n✅ Browser runner started successfully');

      // Handle graceful shutdown
      process.on('SIGINT', () => {
        console.log('\n⚠️  Shutting down...');
        if (result.serverEvents) {
          result.serverEvents.stop();
        }
        process.exit(0);
      });
    })
    .catch((error) => {
      console.error('❌ Failed to start browser runner:', error);
      process.exit(1);
    });
}
